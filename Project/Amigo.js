...
const ultTable = guides => {
    console.log(" alguma coisa");
    let allHtml = "";
    guides.forEach(guide => {
        console.log(guide.start_date);
        const html = `
            <tr>
                <td>${new Date(guide.start_date).toLocaleDateString('pt-BR')}</td>
                <td>${guide.number}</td>
                <td>
                    <img style="width: 25px; height: 25px; margin-right: 5px;border-radius: 50%; border-style : solid #EEE;}" src="${guide.patient.thumb_url || 'https://via.placeholder.com/150x150.jpg'}"/>
                    ${guide.patient.name}
                </td>
    ...
    bodyTable.innerHTML = allHtml;
}
function filter() {
    const inputValue = document.getElementById('filterConv').value.toLocaleLowerCase();
    const guidesFiltered = data.guides.filter(guide => {
        return guide.patient.name.toLocaleLowerCase().includes(inputValue);
    });
        console.log(guidesFiltered)
}
